class PortfolioManager {
  ...
  createProjetoElement(projeto) {
    ...
    if (projeto.projetos && projeto.projetos.length > 0) {
      projeto.projetos.forEach((subProjeto) => {
        ...
        subProjetoDiv.className =
          "p-2 hover:bg-gray-50 cursor-pointer border-t border-gray-100 first:border-t-0";
  ...
  createExtraElement(extra) {
    const div = document.createElement("div");
    div.className =
      "p-3 rounded-lg border border-gray-200 hover:border-purple-300 hover:bg-purple-50 cursor-pointer transition-all duration-200";
  ...
  loadProjeto(link, projeto, subProjeto) {
    ...
    this.updateContentTitle(
      `${projeto.titulo} - ${subProjeto.titulo}`,
      projeto.descricao
    );
  ...
  setCurrentIndex(type, link) {
    const index = this.allItems.findIndex(
      (item) => item.type === type && item.link === link
    );
  ...
  showHomePage() {
    ...
    this.updateContentTitle(
      ...
      "Selecione um módulo ou projeto para começar"
    );
